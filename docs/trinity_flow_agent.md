...
**As delivered in reference integrations (conceptually).**

- End-to-end **three-model** path from bounded text excerpt to **structured cards** with **fused scores**.
- **JSON** extraction from model outputs with **schema validation**.
- **Optional bounded** enricher+judge **retries** when judgment confidence is low.
- **Environment-driven** configuration (model IDs, thresholds, max atoms, input length caps).
- **Logging** hooks for debug vs. production verbosity.

**Planned (engineering and research).**

- **UUID** or other globally unique stable identifiers for every artifact (industry-standard traceability).
- **Mandatory** Trinity path in strict deployments (no silent fallback).
- **Zero-score quarantine**: retain all attempts in storage with **explicit reasons** while **blocking** promotion.
- **Pinned** model revisions and **health checks** before serving.
- **Golden datasets** and regression tests on parse rate and score distributions.
- **Structured telemetry** (metrics, not only logs).
...
*Document type: conceptual specification and scientific narrative. Not tied to a single application repository; implementation details may vary while preserving the stages, schemas, and fusion semantics described above.*
